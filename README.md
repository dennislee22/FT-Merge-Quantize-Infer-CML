...
2. The experiments in chapter 3 onwards were carried out to demonstrate end-to-end lifecycle of fine-tuning a Transformers-based model with specific datasets, merge, quantize it and finally inference.fine-tune a model. `Text-to-SQL` dataset is selected to do supervised learning with the correct prompt to train the model translating plain English into SQL query statement. Here is the brief description of the codes. <br><br>
